	...
	def formattree(self, tree, modname, parent=None):
		...
		for entry in tree:
			if type(entry) is type(()):
				c, bases = entry
				result += '<dt><font face="helvetica, arial"><small>'
...
class TextDoc(Doc):
	...
	def formattree(self, tree, modname, parent=None, prefix=''):
		...
		for entry in tree:
			if type(entry) is type(()):
				c, bases = entry
				result += prefix + classname(c, modname)
				if bases and bases != (parent,):
	...
	def docmodule(self, object, name=None, mod=None):
		...
		try:
			...
			file = '(built-in)'
		result += self.section('FILE', file)
		if desc:
			result += self.section('DESCRIPTION', desc)
		...
		if classes:
			...
			result += self.section('CLASSES', '\n'.join(contents))

		if funcs:
			contents = []
			for key, value in funcs:
				contents.append(self.document(value, key, name))
			result += self.section('FUNCTIONS', '\n'.join(contents))

		if data:
			contents = []
			for key, value in data:
				contents.append(self.docother(value, key, name, 70))
			result += self.section('DATA', '\n'.join(contents))

		if hasattr(object, '__version__'):
			version = str(object.__version__)
			if version[:11] == '$' + 'Revision: ' and version[-1:] == '$':
				version = version[11:-1].strip()
			result += self.section('VERSION', version)
		if hasattr(object, '__date__'):
			result += self.section('DATE', str(object.__date__))
		if hasattr(object, '__author__'):
			result += self.section('AUTHOR', str(object.__author__))
		if hasattr(object, '__credits__'):
			result += self.section('CREDITS', str(object.__credits__))
		return result
...
''')
	...
	def help(self, request):
		if type(request) is type(''):
			...
			elif self.keywords.has_key(request):
				self.showtopic(request)
			elif self.topics.has_key(request):
				self.showtopic(request)
			elif request:
		...
		for modname in sys.builtin_module_names:
			if modname != '__main__':
				...
				if key is None:
					...
					desc = (__import__(modname).__doc__ or '').split('\n')[0]
					if (modname + ' - ' + desc).lower().find(key) >= 0:
...
def ispath(x):
	return type(x) is types.StringType and x.find(os.sep) >= 0
